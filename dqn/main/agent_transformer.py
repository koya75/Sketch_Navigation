...
from models.transformer_model import SKT_model
...
class Agent:
    def __init__(self, args):
        ...
        self.args.save_direct = self.args.save_direct + "SKT/"
        ...
        if self.args.load_dir is None:
            ...
            log_f_name = self.args.save_direct + '/SKT'
        ...
        self.Q = SKT_model(args=self.args, device=self.device).to(self.device)
        self.Q_target = SKT_model(args=self.args, device=self.device).to(self.device)
        ...
        for epoch in range(self.args.n_epochs):
            ...
            for _ in range(self.n_cycles):
                ...
                for _ in range(self.args.n_updates):
                    ...
                    print('\r'+'['+str(time_taken)+'] SKT:' + str(self.run_num) + \
    ...
    def demo_agent(self):
        ...
        for epi in range(self.args.n_test):
            ...
            raw_list, raw_point_list, en_list, de_list, T_Qs = [], [], [], [], []
            if self.args.rand_test:
                rand = torch.tensor([epi+10]).repeat(self.args.num_envs,1)#
            else:
            ...
            print('{} score:{:.2f} sketch:{} way_score:{:.2f}'.format(epi+1, -way_rew[0], rand[0].item(), way_score/self.args.ep_len))
            self.log_f.record_eval_log(epi+1, score.clone().detach().cpu().numpy().round(decimals=2), (way_score/self.args.ep_len).clone().detach().cpu().numpy().round(decimals=2))
            ...
            cv2.imwrite(epi_dir + "/raw_img_point/raw_{0:06d}.png".format(0), raw_point_list[0])
            raw_list, raw_point_list, en_list, de_list = np.array(raw_list), np.array(raw_point_list), np.array(en_list), np.array(de_list)
            ...
            for idx in tqdm(range(len(raw_list))):
                ...
                raw_point_img = raw_point_list[idx]
                en_att = en_list[idx]
                ...
                cv2.circle(raw_point_img, (pattern_point[0,idx,0],pattern_point[0,idx,1]), 1, (0,0,0), -1)
                cv2.imwrite(epi_dir + "/raw_img_point/raw_{0:06d}.png".format(idx), raw_point_img)
        ...
        self.log_f.record_eval_log("avg", avg_score.clone().detach().cpu().numpy().round(decimals=2), avg_way_score.clone().detach().cpu().numpy().round(decimals=2))
        self.env.close()
