...
from models.transformer_model import SKT_nopos_model
from models.cnn_model import CNN_SKT__nopos_model
...
class Agent:
    def __init__(self, args):
        ...
        self.args.save_direct = self.args.save_direct + "SKT_nopos_super"
        ...
        if self.args.load_dir is None:
            ...
            log_f_name = self.args.save_direct + '/SKT_nopos_super'
        ...
        self.Q = SKT_nopos_model(args=self.args, device=self.device).to(self.device)
        self.Q_target = SKT_nopos_model(args=self.args, device=self.device).to(self.device)
        self.Q_target.load_state_dict(self.Q.state_dict())
        self.TTQ = CNN_SKT__nopos_model(obs_shape=args.obs_shape, n_actions=args.action_size, device=self.device).to(self.device)
        pretrain_dir = "results/DQN/Vanilla_nopos/pretrain/" + args.super_model + "/models/best_model.pt"
        ...
        for epoch in range(self.args.n_epochs):
            ...
            for _ in range(self.n_cycles):
                ...
                for _ in range(self.args.n_updates):
                    ...
                    print('\r'+'['+str(time_taken)+'] SKT_nopos_super:' + str(self.run_num) + \
                        ' epoch:'+str(epoch+1) +' episode:'+ str(self.episode_count) + ' alpha:' + str(round(self.alpha,3)), end='', flush=True) # + ' episilon:' + str(round(self.epsilon,3))
                self.update_alpha()
            torch.save(self.Q.state_dict(), self.save_dir + "model.pt")
            reward=self.eval_agent()
            self.log_f.record_train_ttq_log(self.episode_count, reward.round(decimals=2), loss.item(), q_loss.item(), ttq_loss.item())
            self.log_f._train_log_ttq_plot()
    ...
    def update_network(self):
        ...
        with torch.cuda.amp.autocast():
            ...
            with torch.no_grad():
                ...
                ttq_sk = self.TTQ.create_query(pattern)
                ttq = self.TTQ(obs, ttq_sk, t)[batch_idx, action]
                #ttq = self.TTQ(obs).max(1)[0]

            q_target = next_q * self.args.gamma + reward
            q = self.Q(obs, sk, t)[batch_idx, action]
            q_loss = self.MSEloss(q, q_target)# * (1 - self.alpha)
    ...
    def demo_agent(self):
        ...
        for epi in range(self.args.n_test):
            ...
            for idx in tqdm(range(len(raw_list))):
                ...
                make_de_img(q_val, de_att * 255, idx, self.args.action_size, epi_dir)#q_val, 
