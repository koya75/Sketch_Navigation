...
from models.transformer_model import AQT_skt_model
...
class Agent:
    def __init__(self, args):
        ...
        self.args.save_direct = self.args.save_direct + "AQT_skt"
        ...
        if self.args.load_dir is None:
            ...
            log_f_name = self.args.save_direct + '/AQT_skt'
        ...
        self.Q = AQT_skt_model(obs_shape=args.obs_shape, n_actions=args.action_size, device=self.device).to(self.device)
        self.Q_target = AQT_skt_model(obs_shape=args.obs_shape, n_actions=args.action_size, device=self.device).to(self.device)
        ...
        for epoch in range(self.args.n_epochs):
            ...
            for _ in range(self.n_cycles):
                ...
                for _ in range(self.args.n_updates):
                    ...
                    print('\r'+'['+str(time_taken)+'] AQT_skt:' + str(self.run_num) + \
    ...
    def update_network(self):
        if self.buffer.len() < self.args.start_train:
            return
        obs, next_obs, action, reward, ang, next_ang, px, npx, py, npy, pattern, _ = self.buffer.sample()
    ...
    def demo_agent(self):
        ...
        print('\n avg_score:{:.2f} \n run_num:{} best_epi:{} best_score{:.2f}'.format(avg_score, self.run_num, best_epi, best_score))
