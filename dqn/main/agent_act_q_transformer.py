...
class Agent:
    def __init__(self, args):
        ...
        self.args.save_direct = self.args.save_direct + "AQT"
        ...
        if self.args.load_dir is None:
            ...
            log_f_name = self.args.save_direct + '/AQT'
    ...
    """def get_action(self,obs,train=True):
        if random.random() < self.epsilon and train:
            ...
            img = torch.tensor(obs[0], dtype=torch.float32).to(self.device)
            ang = torch.tensor(obs[1], dtype=torch.float32).to(self.device)
            pox = torch.tensor(obs[2], dtype=torch.float32).to(self.device)
            poy = torch.tensor(obs[3], dtype=torch.float32).to(self.device)
            obs_input = [img, ang, pox, poy]
            action = self.Q.get_det_action(obs_input)
        return action
